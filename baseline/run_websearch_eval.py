#!/usr/bin/env python3
"""
WebSearch LLM Baseline Evaluation Runner
测试10个样本，保存完整输出（包含所有tokens、推理过程、搜索来源）
"""
...
from web_search_llm.web_search_llm_baseline import WebSearchLLMBaseline


# 8-Module aligned system prompt (与RAG/Direct LLM一致)
...
请通过网络搜索获取最新循证医学证据，生成一份专业、完整的MDT诊疗报告，确保涵盖所有8个模块。"""


class WebSearchBaselineEvaluator:
    """Evaluator for WebSearch LLM baseline with full output recording"""
    ...
    def run_websearch_baseline(self, patient_info: str) -> Dict:
        """Run WebSearch LLM baseline for a patient with full output"""
        print("  Running WebSearch LLM baseline...")
        # 启用save_full_output以保存完整原始输出
        ...
        return {
            ...
            "full_output": result.get("full_output", {}),  # 完整原始API输出
            "timestamp": datetime.now().isoformat()
        }

    def evaluate_patient(self, row: pd.Series, output_dir: str) -> Dict:
        """Evaluate a single patient with WebSearch LLM"""
        ...
        # Run WebSearch LLM baseline
        websearch_result = self.run_websearch_baseline(patient_info)
        ...
        output_file = os.path.join(output_dir, f"{patient_id}_websearch_result.json")
    ...
    def run_evaluation(self, csv_path: str, output_dir: str = "baseline/results/websearch",
                       max_patients: int = 10):
        """Run full evaluation on patients"""
        ...
        # Save summary
        summary_file = os.path.join(output_dir, "websearch_evaluation_summary.json")
        with open(summary_file, 'w', encoding='utf-8') as f:
            json.dump({
                ...
                "features": [
                    ...
                    "full_output_recorded"
                ]
            }, f, ensure_ascii=False, indent=2)

        print(f"\n{'='*60}")
        print(f"WebSearch LLM Evaluation Complete!")
...
def main():
    """Main entry point"""
    evaluator = WebSearchBaselineEvaluator()

    # Run evaluation on 10 patients
    csv_path = "baseline/test_patients.csv"
    results = evaluator.run_evaluation(csv_path, max_patients=10)

    print("\nWebSearch LLM baseline evaluation completed successfully!")
    print(f"Each result file contains:")
    print("  - patient_info: 输入的患者信息")
    print("  - websearch_result.report: MDT报告")
    print("  - websearch_result.reasoning: 推理过程")
    print("  - websearch_result.search_sources: 搜索来源")
    print("  - websearch_result.full_output: 完整原始API输出（包含所有tokens）")
