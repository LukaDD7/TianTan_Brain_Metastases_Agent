...
def parse_pdf_to_markdown(
    ...
    try:
        ...
        pipe_result = analyze_result.pipe_ocr_mode(
            ...
            debug_mode=True  # 必须开启以获取 raw json
        )
        ...
        # ===== 提取图片-页码映射 =====
        image_page_map = extract_image_page_mapping(raw_data, md_content, output_dir)
...
def extract_image_page_mapping(raw_data: Dict, md_content: str, output_dir: str) -> Dict[str, Any]:
    """
    从 MinerU 底层 JSON 结果中精准提取图片与页码的映射关系。
    搜索范围：images + para_blocks + tables 字段
    """
    images = []
    img_page_map = {}

    def find_image_paths(obj, page_idx: int):
        """递归查找所有图片路径"""
        paths = []
        if isinstance(obj, dict):
            if 'image_path' in obj:
                paths.append((obj['image_path'], page_idx))
            elif 'img_path' in obj:
                paths.append((obj['img_path'], page_idx))
            for v in obj.values():
                paths.extend(find_image_paths(v, page_idx))
        elif isinstance(obj, list):
            for item in obj:
                paths.extend(find_image_paths(item, page_idx))
        return paths

    try:
        # 1. 从所有可能的字段中提取图片页码
        if raw_data and 'pdf_info' in raw_data:
            for page in raw_data['pdf_info']:
                page_idx = page.get('page_idx', 0)

                # 搜索 images 字段
                for img in page.get('images', []):
                    for path, pidx in find_image_paths(img, page_idx):
                        img_name = os.path.basename(path)
                        img_page_map[img_name] = pidx + 1

                # 搜索 para_blocks 字段（段落中的图片）
                for block in page.get('para_blocks', []):
                    for path, pidx in find_image_paths(block, page_idx):
                        img_name = os.path.basename(path)
                        img_page_map[img_name] = pidx + 1

                # 搜索 tables 字段（表格中的图片）
                for table in page.get('tables', []):
                    for path, pidx in find_image_paths(table, page_idx):
                        img_name = os.path.basename(path)
                        img_page_map[img_name] = pidx + 1

        # 2. 扫描 Markdown 中的图片引用
        ...
        if os.path.exists(images_dir):
            actual_images = set(f for f in os.listdir(images_dir) if f.endswith(('.jpg', '.png')))
        ...
        # 5. 处理 Markdown 中没有引用但实际存在的图片
        for img_name in actual_images - matched_images:
            exact_page = img_page_map.get(img_name, 1)
            note = "未在 Markdown 中找到引用" if img_name not in img_page_map else ""
            images.append({
                "image_name": img_name,
                "image_path": os.path.join(output_dir, "images", img_name),
                "relative_path": f"images/{img_name}",
    ...
    exact_count = sum(1 for img in unique_images if not img.get("inferred"))
    inferred_count = sum(1 for img in unique_images if img.get("inferred"))
    ...
    return {
        ...
        "page_range": {
            "start": min((img["page_num"] for img in unique_images), default=1),
            "end": max((img["page_num"] for img in unique_images), default=1)
        },
        "note": f"页码提取：{exact_count}张精准，{inferred_count}张推断"
    }
